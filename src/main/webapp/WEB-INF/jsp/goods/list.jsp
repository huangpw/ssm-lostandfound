    ...
    <div class="layui-row layui-col-space15">
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">失物列表</div>
                <div class="layui-card-body">
                    <form action="<%=path%>/goods/list" method="post" id="searchForm">
                        <input type="hidden" name="pageNum" id="pageNum">
                        <div class="layui-form-mid">关键词:</div>
                        <div class="layui-input-inline" style="width: 100px;">
                            <input type="text" name="goods_name" placeholder="物品名称" value="${requestScope.record.goods_name}"
                                   style="height:33px;" class="layui-input">
                        </div>

                        <div class="layui-input-inline" style="width: 100px;">
                            <input type="text" name="create_date" id="create_date" placeholder="拾到日期"
                                   value="${requestScope.record.create_date}" style="height:33px;" class="layui-input">
                        </div>
                        ...
                        <c:if test="${requestScope.pageInfo.list!=null}">
                            <c:forEach items="${requestScope.pageInfo.list}" var="row">
                                <tr>
                                    ...
                                    <td class="filename" style="display: none;">${row.gods_img}</td>
                                    <td>${row.goods_name}</td>
                                    <td style="width: 80px;text-align: center;">
                                        <img src="${pageContext.request.contextPath}/upload/${row.gods_img }"
                                    ...
                                    <td style="text-align: center">
                                        ...
                                        <a class="layui-btn layui-btn-normal layui-btn-sm"
                                           href="${pageContext.request.contextPath}/goods/query?id=${row.id}"
                ...
                layer.confirm('确认批量删除该记录吗?', {
                    btn: ['是', '否'], btn1: function () {
                        $.ajax({//利用ajax发出请求
                            ...
                            /*dataType:"text",
                            data:{"ids":check_val},*/
                            url: "${pageContext.request.contextPath}/goods/deletemany?ids=" + check_val,
                        ...
                        success: function (data) {
                            ...
                            if (data == "ok") {
                                layer.msg('删除成功', {
                                    ...
                                    window.location.href = "${pageContext.request.contextPath}/goods/list";//相当于刷新界面
                                });
...
<script language="javascript">
    ...
    function deleteRow(id, filename) {
        ...
        if (confirm('您确定删除该条记录吗？')) {
            window.location.href = "${pageContext.request.contextPath}/goods/delete?id=" + id + '&filename=' + filename;
        }
    }
    // 分页查询
